...
export const filterByRiskLevel = (item: PortfolioItem, riskLevel: string | null): boolean => {
  ...
  return item.calif.trim().toUpperCase() === riskLevel.trim().toUpperCase();
};
...
export const generateFilterOptions = (data: PortfolioItem[]): FilterOptions => {
  ...
  data.forEach(item => {
    if (item.calif) {
      uniqueRiskLevels.add(item.calif.trim().toUpperCase());
    }
  });
  const riskLevels = ['Todos', ...Array.from(uniqueRiskLevels).sort()];
